                ...
                if let channelSid: String = channel.sid {
                    ...
                    if let members: TCHMembers = channel.members {
                        members.members { (result, paginator) in
                            if let channelMembers: [TCHMember] = paginator?.items() {
                                channelMembers.forEach({ (member) in
                                    membersList.append(member.identity ?? "");
                                })
                            }
                        }
                    }
            ...
            channel.messages?.getLastWithCount(1, completion: { (response, messages) in
                if response.isSuccessful() {
                    if (messages?.count ?? 0) > 0 {
                        ...
                        object["timestamp"] = messages?.first?.timestamp ?? ""
